'use client'

import { ReactNode, useEffect } from 'react'

import { pageInLoadingAnimation } from '@/utils/animations'

type Props = {
  children: ReactNode
}

export default function Template({ children }: Props) {
  //*: Đoạn code này để đảm bảo rằng khi component được mount và site đã được tải xong, animation sẽ được chạy
  useEffect(() => {
    if (document.readyState === 'complete') {
      pageInLoadingAnimation()
    }
  }, [])

  return (
    <div>
      <div id="banner-1" className="min-h-screen bg-neutral-950 z-10 fixed top-0 left-0 w-1/4" />
      <div id="banner-2" className="min-h-screen bg-neutral-950 z-10 fixed top-0 left-1/4 w-1/4" />
      <div id="banner-3" className="min-h-screen bg-neutral-950 z-10 fixed top-0 left-1/2 w-1/4" />
      <div id="banner-4" className="min-h-screen bg-neutral-950 z-10 fixed top-0 left-3/4 w-1/4" />
      {children}
    </div>
  )
}
